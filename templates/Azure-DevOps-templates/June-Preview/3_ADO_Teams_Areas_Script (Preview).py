...
# === CONFIGURATION ===
config = load_config(default_log_file="3_ADO_Teams_Areas_Log.txt", require_process=False)
ADO_ORG_URL = config.ado_org_url
ADO_PROJECT = config.ado_project
PAT = config.pat
EXCEL_FILE = config.excel_file
LOG_FILE = config.log_file
if os.path.basename(LOG_FILE) == "3_ADO_Teams_Areas_Log.txt":
    run_id = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    LOG_FILE = os.path.join(os.path.dirname(LOG_FILE), f"3_ADO_Teams_Areas_Log_{run_id}.txt")
...
def main():
    log(f"Starting Script 3. Log file: {LOG_FILE}")
