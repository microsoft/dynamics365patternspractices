...
# === USER CONFIGURATION ===
config = load_config(default_log_file="4_ADO_Backlog_Config_Log.txt", require_process=True)
...
if os.path.basename(LOG_FILE) == "4_ADO_Backlog_Config_Log.txt":
    run_id = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    LOG_FILE = os.path.join(os.path.dirname(LOG_FILE), f"4_ADO_Backlog_Config_Log_{run_id}.txt")
...
def main():
    ...
    log(f"Starting Script 4. Log file: {LOG_FILE}")
